...
{
    int16_t accgyroval[9];//陀螺仪读出的值都存在这个数组里
    static float AccData[3], GyroData[3]; //暂存单位转化后的值
    //读取磁力计的原始值
...
////////////
volatile float RDrone_R[3][3];//用来存四元数矩阵

void IMU_getInfo(void)
{
    static float q[4];//四元数的四个数的数组
    static float getValue[9];//暂存读出的已经转化过单位的6轴值
    static float q0q0, q0q1, q0q2, q0q3, q1q1, q1q2, q1q3, q2q2, q2q3, q3q3;
    IMU_getValues(getValue);										//获取原始数据
    MahonyAHRSupdate(getValue[3] * PI / 180, getValue[4] * PI / 180, getValue[5] * PI / 180, //*PI/180 °/s转换成弧度/s
    ...
    //地理坐标系下的加速度                         getValue[0][1][2]是已经转换过单位的载体坐标系的加速度的值
    ...
    //原始角速度数据 角度 陀螺仪直接测出来进行单位转化后的值 单位是°/s
